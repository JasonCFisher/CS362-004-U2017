...
/*Changed conditional check to <=.  This will cause 4 cards to be drawn*/
void smi(struct gameState *state, int handPos, int currentPlayer) {
	//+3 Cards
	int i;
	for (i = 0; i <= 3; i++)
...
/* Changed numActions effect to = 2 instead of numActions+2
...
/*Commented out else statement that removes non-treasure cards from hand and places
/ them in the temp hand.  This will leave all cards drawn in the hand.*/
void adv(struct gameState *state, int handPos, int currentPlayer, int z, int temphand[], int drawntreasure, int cardDrawn) {
